...
NUCLEOTIDES = ['A', 'C', 'G', 'T']
IMPACT_FACTORS = [1, 2, 3, 4]

def solution(s, p, q)
  impact = impact_factor(s)
  integral = selective_integral(impact)

  result = p.length.times.collect do |i|
    if p[i] == q[i]
        next impact[p[i]]
    else
      next IMPACT_FACTORS.each do |f|
        break f unless integral[f][p[i]] == integral[f][q[i]]
      end
    end
  end
...
def impact_factor(s)
  s.split("").map{ |nucleotide| IMPACT_FACTORS[NUCLEOTIDES.index(nucleotide)]}
end

def selective_integral(s)
  result = Hash[IMPACT_FACTORS.collect{ |k| [k, []] }]

  s.each do |factor|
    result.each{ |k, v| result[k] << (v.last || 0).ceil }
