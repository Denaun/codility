...
def solution(a)
  # The first and last elements can never be considered
  a.shift
  a.pop

  first_slice = max_slice(a)
  second_slice = max_slice(a.reverse).reverse
  total = a.length.times.collect{ |i| first_slice[i] + second_slice[i] }
  p first_slice
  p second_slice
  p total
...
def max_slice(a)
  max_ending = Array.new(a.length, 0)
  (a.length-1).times do |index|
    max_ending[index+1] = [max_ending[index]+a[index], 0].max
  end
